...
public class Ejecutor2 {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < nombres.length; i++) {
            if (edad[i] >= 0 && edad[i] <= 10) {
                Persona p = new Persona(nombres[i], apellidos[i], edad[i]);

                PasajeMenorEdad paMenor = new PasajeMenorEdad(pasajeFijo) {
                };
                paMenor.establecerPersona(p);
                pasajes.add(paMenor);
            ...
            if (edad[i] > 18 && edad[i] <= 25) {
                ...
                PasajeUniversitario paUniversidad = new PasajeUniversitario(pasajeFijo) {
                };
                paUniversidad.establecerPersona(p);
                pasajes.add(paUniversidad);

            }
            if (edad[i] > 25 && edad[i] < 65) {
                Persona p = new Persona(nombres[i], apellidos[i], edad[i]);

                PasajeNormal paNormal = new PasajeNormal(pasajeFijo) {
                };
                paNormal.establecerPersona(p);
                pasajes.add(paNormal);

            }
            if (edad[i] >= 65) {
                Persona p = new Persona(nombres[i], apellidos[i], edad[i]);

                PasajeTerceraEdad paTerceraE = new PasajeTerceraEdad(pasajeFijo) {
                };
                paTerceraE.establecerPersona(p);
                pasajes.add(paTerceraE);
